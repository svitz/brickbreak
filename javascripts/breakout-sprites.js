;Quintus.BreakoutSprites = function(Q) {
    ...
    collide: function(col) {
      if(col.obj.isA("Paddle")) {
        ...
        if(col.obj.isA("Block")) {
          Q.audio.play("cblip.ogg");
          col.obj.play("hit",2);
        }
        if(col.obj.isA("Ball")) {
          Q.audio.play("hitpaddle.ogg");
        }
      }
  ...
  Q.Sprite.extend("Block", {
    ...
    destroyed: function() {
      ...
      } else if(rand == 3 && Q("Mini").length == 0) {
        this.stage.insert(new Q.Mini({ x: this.c.x,
                                          y: this.c.y }));
